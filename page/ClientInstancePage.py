...
def expand_server_number(page, instanceId=expandInstanceId, duration=2, sums='1'):
    ...
    with allure.step('实例id输入框输入需要扩容的实例Id'):
        page.fill(ClientCmInstanceList().InstanceIdInputXpath(), instanceId)
        write_log_to_allure_report(page, f'输入实例Id：{instanceId}')

    with allure.step('点击查询按钮进行实例查询'):
        page.click(ClientCmInstanceList().QueryButtonXpath())
        write_log_to_allure_report(page, "点击查询按钮进行实例查询")
        sleep(1)

    with allure.step('获取扩容前实例下差分账号个数'):
        number = page.query_selector(ClientCmInstanceList().InstanceFormalAccountNumberSumsXpath(instanceId)).text_content()
        write_log_to_allure_report(page, f"获取扩容前实例下差分账号个数：{number}")

    with allure.step('点击实例的扩容按钮，进入扩容详情页面'):
        page.click(ClientCmInstanceList().InstanceExpandAccountNumberButtonXpath(instanceId))
    ...
    with allure.step('填写扩容数量'):
        page.fill(ClientInstanceExpandBase().SumInputXpath(), sums)
